...
with main_col2:
    ...
    if dft_output is not None:
        ...
        with file_col2:
            species_to_plot = st.multiselect("Show species", options=dft_output.data['species'], default=dft_output.data['species'])
        ...
        with config_col1:
            plot_cell = st.checkbox("Unit cell", value=True, key="plot_cell")
        with config_col2:
            plot_bonds = st.checkbox("Bonds", value=False, key="plot_bonds")
        with config_col3:
            plot_magmom = st.checkbox("Magnetic moments", value=True, key="plot_magmom")

        magmom_col1, magmom_col2 = st.columns(2)
        with magmom_col1:
            min_magmom = st.number_input("Minimum magnetic moment", min_value=0.0, max_value=10.0, value=0.001, step=0.1, format='%0.4f', key="min_magmom")
        with magmom_col2:
            max_magmom = st.number_input("Maximum magnetic moment", min_value=0.0, max_value=10.0, value=1.0, step=0.1, format='%0.4f', key="max_magmom")

        atom_col1, atom_col2 = st.columns(2)
        with atom_col1:
            zoom_factor = st.number_input("Zoom factor", min_value=0.0, max_value=5.0, value=1.0, step=0.1, key="zoom_factor")
        with atom_col2:
            atom_size = st.number_input("Atom size", min_value=0.1, max_value=2.0, value=0.7, step=0.1, key="atom_size")

        # View angles
        rot_col1, rot_col2, rot_col3 = st.columns(3)
        with rot_col1:
            rot_x = st.number_input("Rotation-X", min_value=-180, max_value=180, value=-85, step=5, key="rot_x")
        with rot_col2:
            rot_y = st.number_input("Rotation-Y", min_value=-180, max_value=180, value=0, step=5, key="rot_y")
        with rot_col3:
            rot_z = st.number_input("Rotation-Z", min_value=-180, max_value=180, value=15, step=5, key="rot_z")

        # Arrows
        arr_col1, arr_col2, arr_col3 = st.columns(3)
        with arr_col1:
            arrow_scale_factor = st.number_input("Arrow length", min_value=1, max_value=50, value=10, step=5, key="arrow_scale_factor")
        with arr_col2:
            arrow_radius = st.number_input("Arrow radius", min_value=0.05, max_value=0.5, value=0.3, step=0.05, key="arrow_radius")
        with arr_col3:
